...
class TorrentParser(object):
    ...
    class _TorrentStr(object):
        ...
        def next_char(self):
            self.curr_char = self.torr_str.read(
                1
        ...
        def parse_int(self):
            ...
            if self.next_char() != TorrentParser.INT_START:
                raise ParsingError(
                    "Error while parsing for an integer. Found %s at position %d while %s is expected."
            ...
            while True:
                ...
                if parsed_int_char not in string.digits:
                    if parsed_int_char != delimiter:
                        raise ParsingError(
                            "Invalid character %s found after parsing an integer (%s expected) at position %d."
    ...
    def get_files_details(self):
        ...
        if files_info:  # 'info' should be present in all torrent files. Nevertheless..
            ...
            if multiple_files_info:  # multiple-file torrent
                ...
                parsed_files_info.append(
                    (
                        self.parsed_content.get("name"),
                        self.parsed_content.get("length"),
                    )
                )
            else:
                return None
